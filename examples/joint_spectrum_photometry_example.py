...
true_params = {
    'redshift': 0.01,
    'f_nickel': 0.1,
    'mej': 1.0,       # ejecta mass (solar masses)
    'vej': 10000.0,   # ejecta velocity (km/s)
...
for band in bands_list:
    ...
    true_mags = supernova_models.arnett(
        phot_times,
        **true_params,
        **model_kwargs
    )
...
true_spectrum_flux = spectral_models.blackbody_spectrum_at_z(
    wavelengths,
    redshift=true_spectrum_params['redshift'],
    rph=true_spectrum_params['r_phot'],
    temp=true_spectrum_params['temperature']
)
...
priors['f_nickel'] = bilby.core.prior.Uniform(0.01, 0.5, 'f_nickel',
                                              latex_label=r'$f_{\rm Ni}$')
priors['mej'] = bilby.core.prior.Uniform(0.1, 3.0, 'mej',
                                         latex_label=r'$M_{\rm ej}$ [$M_\odot$]')
priors['vej'] = bilby.core.prior.Uniform(5000, 20000, 'vej',
                                         latex_label=r'$v_{\rm ej}$ [km/s]')
priors['kappa'] = bilby.core.prior.Uniform(0.5, 10.0, 'kappa',
                                           latex_label=r'$\kappa$ [cm$^2$/g]')
priors['kappa_gamma'] = bilby.core.prior.Uniform(0.01, 1.0, 'kappa_gamma',
                                                 latex_label=r'$\kappa_\gamma$')
...
def photometry_model(time, redshift, f_nickel, mej, vej, kappa, kappa_gamma, temperature_floor, **kwargs):
    return supernova_models.arnett(
        time, redshift=redshift, f_nickel=f_nickel, mej=mej, vej=vej,
        kappa=kappa, kappa_gamma=kappa_gamma, temperature_floor=temperature_floor,
        **kwargs
    )

def spectrum_model(wavelength, temperature, r_phot, redshift, **kwargs):
    return spectral_models.blackbody_spectrum_at_z(
        wavelength, temp=temperature,
        rph=r_phot, redshift=redshift, **kwargs
    )
...
spec_1d_likelihood = redback.likelihoods.GaussianLikelihood(
    ...
    function=lambda wave, temperature_1d, r_phot_1d, **kw:
        spectral_models.blackbody_spectrum_at_z(
            wave, temp=temperature_1d, rph=r_phot_1d, **kw
        )
)
