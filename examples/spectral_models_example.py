...
These tools are particularly useful for analyzing Type Ia supernovae and other
transients with expanding photospheres.
"""
...
# Generate P-Cygni profile with:
# - Rest wavelength: 6355 Angstroms (Si II)
# - Optical depth: 3.0
# - Photospheric velocity: 11,000 km/s
flux_pcygni = spectral_models.p_cygni_profile(
...
ax.plot(wavelength, flux_pcygni, 'b-', linewidth=2)
...
# =============================================================================
# Example 2: Type Ia SN spectrum with multiple P-Cygni lines
# =============================================================================
...
line_list = [
    ...
    {'ion': 'Si II 4130', 'lambda': 4130, 'tau': 2.0},
    {'ion': 'Mg II', 'lambda': 4481, 'tau': 1.5},
    {'ion': 'Fe II 4555', 'lambda': 4555, 'tau': 2.0},
    {'ion': 'Fe II 5018', 'lambda': 5018, 'tau': 2.5},
    {'ion': 'S II W', 'lambda': 5454, 'tau': 1.5},
    {'ion': 'S II 5640', 'lambda': 5640, 'tau': 1.8},
    {'ion': 'Si II 6355', 'lambda': 6355, 'tau': 3.0},
    {'ion': 'Ca II IR', 'lambda': 8579, 'tau': 4.0},
]
...
ax.plot(wavelength_full, spectrum_ia / np.max(spectrum_ia), 'b-', linewidth=1.5)
...
for name, lam in features.items():
    ax.axvline(lam * 1.01, color='red', linestyle='--', alpha=0.3)  # Redshifted
    ax.text(lam * 1.01, 0.95, name, rotation=90, fontsize=9, ha='right')

ax.set_xlabel('Wavelength (Angstroms)', fontsize=12)
ax.set_ylabel('Normalized Flux', fontsize=12)
ax.set_title('Type Ia Supernova Spectrum at Maximum Light (z=0.01)', fontsize=14)
...
flux_physical = spectral_models.p_cygni_profile(
    ...
    v_phot=11000, continuum_flux=1.0
)
...
spectrum = spectral_models.blackbody_spectrum_with_p_cygni_lines(
    wavelength, redshift=0.0, rph=1e15, temp=11000,
    line_list=lines, v_phot=11500  # True velocity: 11,500 km/s
)
...
for (ion_name, rest_wave), color in zip(line_dict.items(), colors):
    ...
    lambda_abs = rest_wave * (1 + v / c_kms)
    ax.axvline(lambda_abs, color=color, linestyle='--', linewidth=2,
               label=f'{ion_name}: {-v:.0f} km/s')
...
for t, v_phot in zip(times, v_phot_evolution):
    ...
    flux = spectral_models.p_cygni_profile(
        ...
        v_phot=v_phot, continuum_flux=1.0
    )
    # Add noise
...
times_measured, velocities, errors = SpectralVelocityFitter.photospheric_velocity_evolution(
    wavelength_list, flux_list, times, line_wavelength=6355, method='min'
)

print("Velocity Evolution (Si II 6355):")
for t, v, err in zip(times_measured, velocities, errors):
...
# Velocity vs time
ax2.errorbar(times_measured, -velocities/1000, yerr=errors/1000,
             fmt='o-', markersize=10, linewidth=2, capsize=5)
ax2.plot(times_measured, -v_phot_evolution/1000, 'r--', linewidth=2,
         alpha=0.7, label='True velocity')
ax2.set_xlabel('Days since maximum', fontsize=12)
ax2.set_ylabel('Photospheric Velocity (1000 km/s)', fontsize=12)
...
ax.set_title('SYNOW-style Line Profile: Effect of Power-Law Index', fontsize=14)
ax.legend(fontsize=11)
ax.grid(True, alpha=0.3)
