...
GRB = '070809'
path = 'GRBData'
data_mode = 'flux'  # Should eventually be luminosity data

# Flux density, flux data
afterglow_data = redback.getdata.get_afterglow_data_from_swift(GRB, data_mode='flux') #, path=path)
# creates a GRBDir with GRB

# create Luminosity data
data = redback.grb.SGRB.from_path_and_grb_with_truncation(path=path, grb=GRB, truncate=True, data_mode=data_mode, truncate_method='analytical')
...
# use default priors
priors = redback.priors.get_priors(model=model, data_mode=data_mode)
...
result = redback.fit_model(name=GRB, model=model, sampler=sampler,
                           path=path, prior=priors, data_mode=data_mode, source_type='SGRB')
