...
def _perform_extinction(flux_density, angstroms, av_host, rv_host, av_mw=0.0, rv_mw=3.1,
    ...
    rv_host
        extinction parameter for host galaxy (default 3.1)
    av_mw
        V-band extinction from Milky Way in magnitudes
    rv_mw
        extinction parameter for Milky Way (default 3.1)
    redshift
        source redshift (needed for host extinction)
    host_law
        extinction law for host galaxy ('fitzpatrick99', 'fm07', 'calzetti00', 'odonnell94', 'ccm89')
    mw_law
        extinction law for Milky Way ('fitzpatrick99', 'fm07', 'calzetti00', 'odonnell94', 'ccm89')
    kwargs
        additional parameters for specific extinction laws
...
def _evaluate_extinction_model(time, av_host, av_mw=0.0, model_type=None, **kwargs):
    ...
    av_mw
        V-band extinction from Milky Way in magnitudes
    model_type
        None, or one of the types implemented
    kwargs
        Must include all parameters for base_model plus: - redshift: source redshift (required for host extinction) - rv_host: host R_V parameter (default 3.1) - rv_mw: MW R_V parameter (default 3.1) - host_law: host extinction law (default 'fitzpatrick99') - mw_law: MW extinction law (default 'fitzpatrick99')

    Returns
    -------
        flux/magnitude with extinction applied
    """
...
def extinction_with_function(time, av_host, **kwargs):
    ...
    Returns
    -------
        set by kwargs['output_format'] - 'flux_density', 'magnitude', 'flux', 'spectra' with extinction applied
    """
...
def extinction_with_afterglow_base_model(time, av_host, **kwargs):
    ...
    Returns
    -------
        set by kwargs['output_format'] - 'flux_density', 'magnitude', 'flux', 'spectra' with extinction applied Note that only sed variant models can take magnitude as an output
    """
...
def extinction_afterglow_galactic_dust_to_gas_ratio(time, lognh, factor=2.21, **kwargs):
    ...
    factor
        factor to convert nh to av i.e., av = nh/factor (default 2.21)
    kwargs
        Must be all the parameters required by the base_model specified using kwargs['base_model'] plus: - redshift: source redshift (required) - av_mw: MW V-band extinction in magnitudes (default 0.0) - rv_host: host R_V parameter (default 3.1) - rv_mw: MW R_V parameter (default 3.1) - host_law: host extinction law (default 'fitzpatrick99') - mw_law: MW extinction law (default 'fitzpatrick99') Available extinction laws: 'fitzpatrick99', 'fm07', 'calzetti00', 'odonnell94', 'ccm89'
...
def _extinction_with_predeceleration(time, lognh, factor, **kwargs):
    ...
    Returns
    -------
        set by output format kwarg - 'flux_density', 'magnitude', 'flux' with extinction applied
    """
