...
def bpl_cooling_envelope_additive(time, redshift, logl, alpha_1, alpha_2, peak_time, logt,
                                  mbh_6, stellar_mass, eta, alpha, beta, **kwargs):
    """
    Additive dual-blackbody TDE model following fitted_pl_decay structure:
    Early emission (fixed-T blackbody with broken power law amplitude) plus cooling envelope
    (evolving-T blackbody from Metzger physics).

    Structure analogous to fitted_pl_decay:
    - Early component = rise + decay (broken power law amplitude, fixed blackbody T)
    - Cooling envelope = physical evolving blackbody (replaces FitTeD plateau)
    - Total = early + envelope (both as νLν, then converted to flux density)

    :param time: observer frame time in days
    :param redshift: redshift
    :param logl: log10 of early component luminosity at reference frequency (erg/s)
    :param alpha_1: power law index for early component rise
    :param alpha_2: power law index for early component decay (should be POSITIVE for decay)
    :param peak_time: peak time for early component in days
    :param logt: log10 of fixed blackbody temperature for early component (Kelvin)
    :param mbh_6: mass of supermassive black hole in units of 10^6 solar mass
    :param stellar_mass: stellar mass in units of solar masses
    :param eta: SMBH feedback efficiency
    :param alpha: disk viscosity
    :param beta: TDE penetration factor
    :param kwargs: Additional parameters
    :param xi: Optional (default=1) when cooling envelope starts: envelope_start = xi * tfb (days)
    :param transition_width: Optional (default=5.0) transition width in days for smooth envelope turn-on
    :param frequency: Required if output_format is 'flux_density'
    :param bands: Required if output_format is 'magnitude' or 'flux'
    :param output_format: 'flux_density', 'magnitude', 'flux', 'spectra'
    :param lambda_array: Optional wavelength array (Angstroms)
    :param cosmology: Cosmology for luminosity distance. Defaults to Planck18
    ...
    binding_energy_const = kwargs.get('binding_energy_const', 0.8)
    tfb_sf = calc_tfb(binding_energy_const, mbh_6, stellar_mass)
    xi = kwargs.get('xi', 1.)
    envelope_start = xi * tfb_sf  # source frame days
    ...
    if kwargs['output_format'] == 'flux_density':
        ...
        # Convert peak time to source frame
        peak_time_sf = peak_time / (1. + redshift)

        # Early component: broken power law amplitude with fixed-T blackbody (gives νLν)
        freq_0 = 6e14
        amplitude = pm.exponential_powerlaw(time=time * cc.day_to_s, a_1=1.,
                                            tpeak=peak_time_sf * cc.day_to_s,
                                            alpha_1=alpha_1, alpha_2=alpha_2)
        l_e_amp = 10 ** logl * amplitude

        nulnus_early = l_e_amp * (frequency / freq_0) ** 4 * \
                       (np.exp(cc.planck * freq_0 / (cc.boltzmann_constant * 10 ** logt)) - 1) / \
                       (np.exp(cc.planck * frequency / (cc.boltzmann_constant * 10 ** logt)) - 1)

        # Cooling envelope component: smoothly turns on after envelope_start
        nulnus_envelope = np.zeros(len(time))
        envelope_start_days = envelope_start / cc.day_to_s  # Convert seconds to days

        # Smooth turn-on using tanh
        transition_width = kwargs.get('transition_width', 5.0)  # days, default 5 days
        time_since_start = time - envelope_start_days
        smooth_weight = 0.5 * (1.0 + np.tanh(time_since_start / transition_width))

        mask = time >= (
                    envelope_start_days - 3 * transition_width)  # Start computing a bit earlier for smooth transition
        if np.any(mask):
            time_env = time[mask] - envelope_start_days
            time_env = np.maximum(time_env, 0.0)  # Don't go negative

            temp_interp = interp1d(output.time_since_fb / cc.day_to_s, output.photosphere_temperature,
                                   bounds_error=False, fill_value=(output.photosphere_temperature[0],
                                                                   output.photosphere_temperature[-1]))
            radius_interp = interp1d(output.time_since_fb / cc.day_to_s, output.photosphere_radius,
                                     bounds_error=False, fill_value=(output.photosphere_radius[0],
                                                                     output.photosphere_radius[-1]))
            T_env = temp_interp(time_env)
            R_env = radius_interp(time_env)

            # Get flux density, convert to νLν - fully vectorized
            fnu = sed.blackbody_to_flux_density(temperature=T_env, r_photosphere=R_env,
                                                dl=dl, frequency=frequency[mask])  # erg/s/Hz/cm²
            # Convert erg/s/Hz/cm² to νLν (erg/s): fν * ν * 4π dl²
            envelope_contribution = fnu.to(uu.erg / uu.s / uu.Hz / uu.cm ** 2).value * frequency[
                mask] * 4.0 * np.pi * dl ** 2

            # Apply smooth weight
            nulnus_envelope[mask] = envelope_contribution * smooth_weight[mask]

        nulnus = nulnus_early + nulnus_envelope
        flux_density = nulnus / (4.0 * np.pi * dl ** 2 * frequency)
        return flux_density / 1.0e-26 / (1 + redshift)

    else:
        # Spectra/magnitude/flux output (same structure as fitted_pl_decay)
        time_obs = time
        lambda_observer_frame = kwargs.get('lambda_array', np.geomspace(100, 60000, 100))
        time_temp = np.geomspace(0.1, 3000, 300)
        ...
        nulnus_early = ((l_e_amp[:, None] * (frequency / freq_0) ** 4 *
                         (np.exp(cc.planck * freq_0 / (cc.boltzmann_constant * 10 ** logt)) - 1) /
                         (np.exp(cc.planck * frequency / (cc.boltzmann_constant * 10 ** logt)) - 1)).T)

        # Cooling envelope component - smoothly turns on
        nulnus_envelope = np.zeros((len(frequency), len(time)))
        envelope_start_days = envelope_start / cc.day_to_s  # Convert seconds to days

        # Smooth turn-on using tanh
        transition_width = kwargs.get('transition_width', 5.0)  # days, default 5 days
        time_since_start = time_temp - envelope_start_days
        smooth_weight = 0.5 * (1.0 + np.tanh(time_since_start / transition_width))

        mask = time_temp >= (envelope_start_days - 3 * transition_width)

        if np.any(mask):
            time_env = time_temp[mask] - envelope_start_days
            time_env = np.maximum(time_env, 0.0)  # Don't go negative
            ...
            # Get blackbody flux density and convert to νLν to match early component
            fnu = sed.blackbody_to_flux_density(temperature=T_env, r_photosphere=R_env,
                                                dl=dl, frequency=frequency[:, None])  # erg/s/Hz/cm²
            # Convert to νLν: multiply by ν then by 4π dl² to get luminosity
            envelope_contribution = (fnu.to(uu.erg / uu.s / uu.Hz / uu.cm ** 2).value *
                                     frequency[:, None] * 4.0 * np.pi * dl ** 2)

            # Apply smooth weight
            nulnus_envelope[:, mask] = envelope_contribution * smooth_weight[mask]

        # Both nulnus arrays in erg/s, convert to flux density in erg/s/Hz/cm²
        flux_density = ((nulnus_early + nulnus_envelope) /
                        (4.0 * np.pi * dl ** 2 * frequency[:, np.newaxis]))
        fmjy = flux_density.T
        spectra = flux_density_to_spectrum(flux_density=fmjy, redshift=redshift,
                                           lambda_observer_frame=lambda_observer_frame)
