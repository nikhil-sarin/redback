...
grb_data_file = os.path.join(os.path.dirname(__file__), "data/GRBs.dat")
grbs, z, alpha, beta, Ecut, obs_elow, obs_ehigh, cts2flux_abs, cts2flux_unabs = np.loadtxt(grb_data_file,
                                                                                           skiprows=2, dtype='|S12',
                                                                                           unpack=True, delimiter=' ')
grbs = ['GRB050319', 'GRB060729', 'GRB061121', 'GRB080430']


# def create_restframe_lc(grb, z, alpha, beta, Ecut, obs_elow, obs_ehigh, bol_elow, bol_ehigh, cts2flx_unabs,
#                         cts2flx_abs):
...
#     print(kcorr)
#
#     z_dl = np.genfromtxt('data/z_dl.dat', skip_header=1)
#     dl = (np.interp(z, z_dl[:, 0], z_dl[:, 1])) * 3.08568e24  # interpolate to find dl and then convert dl into cm
#     fbiso = (dl ** 2.) * 4. * np.pi * kcorr
#     Liso = (obs_lc[:, 3] * (
#                 cts2flx_unabs / cts2flx_abs)) * fbiso * 1e-50  # calculate restframe luminosity in 10^50 erg s^-1 including conversion to unabs
...
#     rest_lc = np.hstack((np.matrix(rest_time).T, np.matrix(rest_time_pos_err).T, np.matrix(rest_time_neg_err).T,
#                          np.matrix(Liso).T, np.matrix(Liso_pos_err).T, np.matrix(Liso_neg_err).T))
#     return rest_lc
#
#
