...
class RedbackResult(Result):
    ...
    def __init__(
        ...
        This result adds the capability of utilising the plotting methods of the `Transient`
        such as `plot_lightcurve`. The class does this by reconstructing the `Transient` object
        that was used during the run by saving the required information in `meta_data`.

        Parameters
        ----------
        label : str, optional
            Labels of files produced by this class (default is 'no_label')
        outdir : str, optional
            Output directory of the result (default is '.')
        ...
        meta_data : dict, optional
            Additional dictionary. Contains the data used during the run and
        ...
        nested_samples : pd.DataFrame, optional
            An array of the unweighted samples
        log_evidence : float, optional
            The log evidence value if provided (default is np.nan)
        log_evidence_err : float, optional
            The log evidence error value if provided (default is np.nan)
        information_gain : float, optional
            The information gain calculated (default is np.nan)
        log_noise_evidence : float, optional
            The log noise evidence (default is np.nan)
        log_bayes_factor : float, optional
            The log Bayes factor if we sampled using the likelihood ratio (default is np.nan)
...
def read_in_result(
    ...
    """
    Load a redback result from file.
    ...
    extension : str, optional
        If filename is not given, filename extension.
        Must be in ('json', 'hdf5', 'h5', 'pkl', 'pickle', 'gz') (default is 'json')
    gzip : bool, optional
