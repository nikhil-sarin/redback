...
class CreateJointPriorTest(unittest.TestCase):
    ...
    def test_create_joint_prior_shared_params(self):
        ...
        # Check that messenger-specific parameters keep their likelihood names
        self.assertIn('kappa', joint_prior)
        self.assertIn('log_n0', joint_prior)
    ...
    def test_create_joint_prior_no_shared_params(self):
        ...
        # All parameters should keep their likelihood names
        self.assertIn('param_a', joint_prior)
        self.assertIn('param_b', joint_prior)

    def test_create_joint_prior_duplicate_nonshared_parameter_raises(self):
        """Duplicate non-shared names cannot represent independent parameters."""
        optical_priors = bilby.core.prior.PriorDict()
        optical_priors['amplitude'] = bilby.core.prior.Uniform(0, 1, 'amplitude')

        xray_priors = bilby.core.prior.PriorDict()
        xray_priors['amplitude'] = bilby.core.prior.Uniform(0, 1, 'amplitude')
...
class MultiMessengerCoreFunctionalityTest(unittest.TestCase):
    ...
    def test_build_likelihood_for_messenger_with_parameter_mapping(self):
        """Parameter mappings expose joint names while calling native model names."""
        mm = MultiMessengerTransient(optical_transient=self.optical_transient)

        def mapped_model(time, thv, amplitude, **kwargs):
            return amplitude * np.exp(-time) + thv

        likelihood = mm._build_likelihood_for_messenger(
            messenger='optical',
            transient=self.optical_transient,
            model=mapped_model,
            parameter_mapping={'viewing_angle': 'thv'}
        )

        self.assertIn('viewing_angle', likelihood.parameters)
        self.assertNotIn('thv', likelihood.parameters)
        self.assertIn('amplitude', likelihood.parameters)
        likelihood.parameters['viewing_angle'] = 0.3
        likelihood.parameters['amplitude'] = 1e-12
        self.assertTrue(np.all(np.isfinite(likelihood.model_output)))

    def test_build_likelihood_for_flux_density_without_active_bands(self):
        """Flux-density transients without active bands use all data."""
        time = np.array([1.0, 2.0, 3.0])
        frequency = np.ones_like(time) * 1e9
        flux_density = np.array([1e-3, 2e-3, 3e-3])
        flux_density_err = 0.1 * flux_density
        transient = Transient(
            time=time,
        ...
        mm = MultiMessengerTransient(radio_transient=transient)

        def model(time, amplitude, **kwargs):
        ...
        self.assertTrue(np.array_equal(likelihood.y, flux_density))

    def test_multimessenger_likelihood_accepts_sampler_parameters(self):
        """The joint wrapper accepts bilby's log_likelihood(parameters=...) call."""
        time = np.array([1.0, 2.0, 3.0])

        def model(time, amplitude):
            return amplitude * np.ones_like(time)

        likelihood_1 = GaussianLikelihood(
            x=time, y=np.ones_like(time), sigma=np.ones_like(time), function=model)
        likelihood_2 = GaussianLikelihood(
            x=time, y=2 * np.ones_like(time), sigma=np.ones_like(time), function=model)
        joint_likelihood = MultiMessengerLikelihood(likelihood_1, likelihood_2)

        log_likelihood = joint_likelihood.log_likelihood(parameters={'amplitude': 1.0})
    ...
    def test_fit_joint_multiple_likelihoods(self, mock_sampler):
        ...
        # Check that the sampler-compatible multimessenger likelihood was created
        call_kwargs = mock_sampler.call_args[1]
        self.assertIsInstance(call_kwargs['likelihood'], MultiMessengerLikelihood)
...
class CreateJointPriorAdvancedTest(unittest.TestCase):
    ...
    def test_create_joint_prior_multiple_shared_params(self):
        ...
        # Non-shared params keep their likelihood names
...
class RealCodePathsTest(unittest.TestCase):
    ...
    def test_fit_joint_constructs_joint_likelihood(self, mock_sampler):
        """Test that a sampler-compatible joint likelihood is constructed."""
        mock_sampler.return_value = mock.MagicMock()
        ...
        # Check that the multimessenger likelihood was constructed and passed to sampler
        call_kwargs = mock_sampler.call_args[1]
...
class EdgeCasesAndWarningsTest(unittest.TestCase):
    ...
    def test_build_quadrature_likelihood_with_time_errors_raises(self):
        """Quadrature-noise likelihoods do not silently ignore time errors."""
        mock_transient_with_xerr = mock.MagicMock(spec=Transient)
        mock_transient_with_xerr.get_filtered_data.return_value = (
    ...
    def test_fit_joint_with_shared_params_logging(self, mock_sampler):
        """Test that valid shared parameters are logged."""
        mock_sampler.return_value = mock.MagicMock()
    ...
    @mock.patch('bilby.run_sampler')
    def test_fit_joint_missing_shared_params_raises(self, mock_sampler):
        """Declared shared parameters must be present in likelihood parameters."""
        mock_sampler.return_value = mock.MagicMock()
    ...
    @mock.patch('bilby.run_sampler')
    def test_fit_joint_singly_present_shared_param_raises(self, mock_sampler):
        """A shared parameter in only one of several likelihoods is not shared."""
        mock_sampler.return_value = mock.MagicMock()
        ...
        mm = MultiMessengerTransient(optical_transient=mock_optical, xray_transient=mock_xray)

        def opt_model(x, shared=1):
            return x * shared

        def xray_model(x, independent=1):
            return x * independent

        priors = bilby.core.prior.PriorDict({
            'shared': bilby.core.prior.Uniform(0, 10),
            'independent': bilby.core.prior.Uniform(0, 10)
        })
    ...
    def test_create_joint_prior_messenger_specific_parameters(self):
        """Test that non-shared params keep names that match likelihood parameters."""
        optical_priors = bilby.core.prior.PriorDict()
        optical_priors['mej'] = bilby.core.prior.Uniform(0.01, 0.1)
        ...
        # All params should match the likelihood parameter names
        self.assertIn('mej', joint_prior)
        self.assertIn('vej', joint_prior)
        self.assertIn('logn0', joint_prior)
        self.assertIn('p', joint_prior)
